...
sandboxApp.get(["/", "/sandbox.html"], (req, res) => {
  ...
  // Use { root } so express's `send` only dotfile-checks the bare filename.
  // Passing an absolute path makes it check every segment, which 404s when the
  // checkout lives under a dot-directory (e.g. a git worktree in .claude/).
